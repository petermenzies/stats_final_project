...
ggplot(krige_joined, aes(x = mean_colonies, y = mean_pred_occurrence)) +
  ...
  theme(plot.margin = margin(.5, .75, .75, .5, "cm"),
...
```{r, fig.align='center'}
lm(mean_pred_occurrence ~ mean_colonies, krige_joined) %>% 
  summary %>% 
  xtable() %>% 
  gt::gt()
```
...
While this analysis yielded a statistically significant result, there are some potential issues that need to be taken into consideration. 
...
G. M. Angelella, C. T. McCullough, and M. E. O’Rourke, “Honey Bee Hives Decrease Wild Bee Abundance, Species Richness, and Fruit Count on Farms Regardless of Wildflower Strips,” Scientific Reports 11, no. 1 (February 5, 2021): 3202, 
